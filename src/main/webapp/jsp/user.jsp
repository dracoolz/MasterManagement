			...
			<table border="1" style="border-collapse: collapse" class="tablesorter" id="user_table">
				...
				<% for(int i=0; i < userList.size(); i++) { %>
					<tr>
						...
						<td><%=userList.get(i).getRoleString()%></td>
						<td><input type ="submit" name="submit" formaction=<%="./DUControl?type=user&id="+userList.get(i).getEmp_id()%> value="変更"></td>
						<td><input type ="submit" name="submit" formaction=<%="./DUControl?type=user&id="+userList.get(i).getEmp_id()%> value="削除"></td>
					</tr>
				<% } %>
			</table>
		</div>
